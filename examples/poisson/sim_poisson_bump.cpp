...
int main(int ac, char* av[]) 
{
    ...
    try {
        ...
        desc.add_options()
            ...
            ("duration", po::value<double>(), "bump duration in s")
            ("width", po::value<double>(), "bump width as stdev in relative units of population size")
	...
	MovingBumpGroup * input_group = new MovingBumpGroup(size, duration, width, kappa);

	sprintf(strbuf, "%s/%s.%d.ras", dir.c_str(), file_prefix.c_str(), sys->mpi_rank() );
	SpikeMonitor * smon_e = new SpikeMonitor( input_group, strbuf, size);

	sprintf(strbuf, "%s/%s.%d.prate", dir.c_str(), file_prefix.c_str(), sys->mpi_rank() );
	PopulationRateMonitor * pmon_e = new PopulationRateMonitor( input_group, strbuf, 1.0 );

	RateChecker * chk = new RateChecker( input_group , -1 , 20.*kappa , 10);
