...
int main(int ac, char* av[]) 
{
	...
	if (infilename.empty()) {
		...
		con_ee = new TripletScalingConnection(neurons_e,neurons_e,
				str.c_str(),tau_hom,eta,kappa,beta_scaling,wmax,GLUT);


		str = infilename;
		str += ".e.nstate";
		neurons_e->load_from_file(str.c_str());
		str = infilename;
		str += ".i.nstate";
		neurons_i->load_from_file(str.c_str());
	...
	if ( corr ) {
		...
		// con_exte->set_block(0,2500,0,size,0);

		CorrelatedPoissonGroup * corr_e = new CorrelatedPoissonGroup(size,stimfreq,50,10e-3);
		// CorrelatedPoissonGroup * corr_i = new CorrelatedPoissonGroup(size,50,50,10e-3);
		corr_e->set_amplitude(50.);
