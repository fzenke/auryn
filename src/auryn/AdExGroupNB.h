...
* Modified by Anders Lansner from AdExGroup
*
...
#ifndef ADEXGROUPNB_H_
#define ADEXGROUPNB_H_
...
/*! \brief Conductance based Adaptive Exponential neuron model -
    Brette and Gerstner (2005), extended with nmda conductance
    mechanisms
 *
 ...
 * 3637–3642.
 * An Nmda conductance is included with a default tau of 100ms. The NMDA current is
 * not dependent on voltage or the AMPA component.
 *
 * */
class AdExGroupNB : public NeuronGroup
{
private:
    AurynFloat e_rest, e_reset, e_rev_gaba, e_rev_ampa,e_thr, g_leak, c_mem, deltat;
    AurynFloat tau_ampa, tau_nmda, tau_gaba, tau_mem;
    AurynFloat scale_ampa, scale_nmda, scale_gaba, scale_mem, scale_w, scale_current;
    AurynFloat * t_w;
    ...
    AurynStateVector * I_exc_ampa;
    AurynStateVector * I_exc_nmda;
...
public:
    ...
    /*! Sets the exponential time constant for the NMDA channel (default 100ms) */
    void set_tau_nmda(AurynFloat tau);

    /*! Gets the exponential time constant for the AMPA channel */
    AurynFloat get_tau_ampa();

    /*! Gets the exponential time constant for the NMDA channel */
