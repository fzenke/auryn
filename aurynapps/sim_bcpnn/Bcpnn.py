...
def plot1(fig = 1) :
    ...
    pospikes = np.loadtxt("pospikes.txt")
    prspikes = np.loadtxt("prspikes.txt")
    ...
    bj = np.loadtxt("bj.txt")
    ax9 = plt.subplot(5,2,9)
    ax9.plot(TT,BBj[:,rj])
    ax9.plot(bj[:,0],bj[:,1]*invbwgain)
    ax9.set_xlim(t1,t2)
    ax9.set_ylim(-5,0)
    ax9.set_title('bj-trace in [0,...]')

    wij = np.loadtxt("wij.txt")
    ax10 = plt.subplot(5,2,10)
    ax10.plot(TT,WWij[:,ri,rj])
    ax10.plot(wij[:,0],wij[:,1]*invbwgain)
    ax10.set_xlim(t1,t2)
    ax10.set_ylim(-5,5)
    ax10.set_title('wij-trace')
